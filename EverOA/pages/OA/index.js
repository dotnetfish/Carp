...
Page( {
  ...
  ,
  onLoad: function() {
    console.log( 'onLoad oa index' )
    var that = this;

    //调用应用实例的方法获取全局数据
    app.getUserInfo( function( userInfo ) {
      //如果未绑定用户则跳转至绑定界面
 console.log( 'getUserInfo oa index success:'+userInfo.spAccount.nickName )
      that.setData( {
        userInfo: userInfo.account,
        wxInfo: userInfo.spAccount
      })
      console.log( 'that data info success:'+that.data.wxInfo.nickName )
    })
